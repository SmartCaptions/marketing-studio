...
timeline as `shots[i].from`). The riser should land 45 frames before the last line starts —
`shots[shots.length - 1].from - 45`. The renderer validates each cue and skips invalid ones
